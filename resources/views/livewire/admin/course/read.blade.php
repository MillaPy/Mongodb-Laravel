    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('Course')) ]) }}</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded" style="background-color: #e9ecef!important;">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __(\Illuminate\Support\Str::plural('Course')) }}</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(config('easy_panel.crud.course.create'))
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.course.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('Course') ]) }}</a>
                        </div>
                        @endif
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                    ...
                    <tr>
                        ...
                        @if(config('easy_panel.crud.course.delete') or config('easy_panel.crud.course.update'))
                        <td>{{ __('Action') }}</td>
                        @endif
                    </tr>

                    @foreach($courses as $course)
                        @livewire('admin.course.single', ['course' => $course], key($course->id))
                    @endforeach
                    </tbody>
                </table>
            </div>
